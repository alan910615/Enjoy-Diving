...
// for shoping cart start
	...
	function count() {
		...
		var tr = document.getElementsByClassName('product');
		var count_price = document.getElementsByClassName('count_price'); //商品的金額
		var countPrice = document.getElementById('countPrice'); //結帳總覽的商品總計
		...
		for(let i = 0; i < tr.length; i++){
			total += parseInt(count_price[i].innerText);
		}
		countPrice.innerText = total;
	};
	count();
	//當頁面load後 再次更新價錢
